...
do
  ...
  scp "$ip":"$FLINKROOT"/flink-dist/target/flink-1.16.2-bin/flink-1.16.2/log/* "$FLINKROOT"/scripts/flinklogs_"$timestamp"/"$ifname"/
done


mv  "$FLINKROOT"/scripts/flinklogs_* /home/tidb/Desktop/toshiba/flinklogs/

#mkdir /users/pentium3/hdd/flinklogs/
#mv  "$FLINKROOT"/scripts/flinklogs_* /users/pentium3/hdd/flinklogs/
