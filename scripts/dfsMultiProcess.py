...
class DFSprocess:
    ...
    def __init__(self, workers_num, workers_slot, ratio, JobGraph, outer_dfs_order, processPoolPara, findBound=False):

        # If this run is to find bound
        self.findBound = findBound

        # Process Pool related para
        self.processPoolPara = processPoolPara
    ...
    def outerDFS(self, ops, curPlace, plans, timeManager):
        # Successfully placed all tasks
        if(len(ops) == 0):
            plans.append( copy.deepcopy(curPlace.map) )
            if self.findBound:
                # let the main process to end the program
                sharedPool.earlyExit()
        ...
        # Call inner DFS
        self.innerDFS(0, nodes, op.parallelism, curPlace.leftSlots, op, None, -1, ops, curPlace, plans, timeManager)
        ops.append(op)

    ############################# Inner DFS #############################

    def innerDFS(self, index, nodes, leftTasks, leftSlots, op, lastNodeType, baseSlot, ops, curPlace, plans, timeManager, deployedOps=None):
        # declare the shared variable
        global sharedPool

        try:
            if deployedOps != None:
                self.deployedOps = deployedOps

            # DEBUG: check if vars in new process align with the parent process. DO NOT use multiprocessing again!!!! 
            # if printID != -1:
            #     print(printID,"--", self.deployedOps)
            ...
            if (leftTasks == 0):    # all tasks of this OP are placed
                ...
                # Now explore next OP
                self.outerDFS(ops, curPlace, plans, timeManager)
            ...
            for num in range(atLeast, atMost+1):
                ...
                if timeManager.timeOut():
                    ...
                    if sharedPool.acquireProcess():
                        ...
                        # printId = 0
                        # with sharedPool.tempLock:
                        #     printId = sharedPool.tempNum.value
                        #     sharedPool.tempNum.value += 1
                        # print(printId,"--", self.deployedOps)

                        args = (index+1, nodes, leftTasks-num, leftSlots-slots, op, nodes[index].key, num, ops, curPlace, [], newTimeManager, self.deployedOps)
                        sharedPool.scheduleProcess(args)
                    # all processes are running -> local execution
                    else:
                        timeManager.updateNextTime()
                        self.innerDFS(index+1, nodes, leftTasks-num, leftSlots-slots, op, nodes[index].key, num, ops, curPlace, plans, timeManager)
                else:
                    self.innerDFS(index+1, nodes, leftTasks-num, leftSlots-slots, op, nodes[index].key, num, ops, curPlace, plans, timeManager)
    ...
    def process(self):
        ...
        sharedPool = SharedPool(self.processPoolPara.numProcess)
        pool = ProcessPool(sharedPool, processInit, self.innerDFS, plans)
        ...
        timeManager = TimeManager(self.processPoolPara.low, self.processPoolPara.high)

        # Explore the 1st OP
        op = ops.pop()
        nodes = self.buildNodeList(curPlace)
        if sharedPool.acquireProcess():
            args = (0, nodes, op.parallelism, curPlace.leftSlots, op, None, -1, ops, curPlace, [], timeManager)
            sharedPool.scheduleProcess(args)

        # Start the main process loop
        earlyExit = pool.start()
        ...
        print("DFS early exit:", earlyExit)
...
#     network_threshold_ratio = 0.5         # worst value * ratio: 1.0 going down to tight the requirement
    ...
    dfsproc=DFSprocess(workers_num, workers_slot, ratio, JobGraph, outer_dfs_order, processPoolPara)
...
def microBenchmark(workers_ip, workers_slot, paras, randomOptimal, JobGraph):
    ...
    while loop<loopLimit:
        ...
        dfsproc=DFSprocess(workers_num, workers_slot, ratio, JobGraph, "compute", processPoolPara, findBound=True)
        # dfsproc=DFSprocess(workers_num, workers_slot, ratio, JobGraph, "default", processPoolPara, findBound=True)
        try:
            optimals, plans=dfsproc.process()
    ...
    if totalIO == 0:        # no need to check if no cost on IO access
        ...
        while loop<loopLimit:
            ...
            dfsproc=DFSprocess(workers_num, workers_slot, ratio, JobGraph, "state", processPoolPara, findBound=True)
            # dfsproc=DFSprocess(workers_num, workers_slot, ratio, JobGraph, "default", processPoolPara, findBound=True)
            try:
                optimals, plans=dfsproc.process()
            except HasAvailablePlan as e:
    ...
    while loop<loopLimit:
        ...
        dfsproc=DFSprocess(workers_num, workers_slot, ratio, JobGraph, "network", processPoolPara, findBound=True)
        # dfsproc=DFSprocess(workers_num, workers_slot, ratio, JobGraph, "default", processPoolPara, findBound=True)
        try:
            optimals, plans=dfsproc.process()
        except HasAvailablePlan as e:
    ...
    while loop<50:
        ...
        dfsproc=DFSprocess(workers_num, workers_slot, ratio, JobGraph, "compute", processPoolPara)
        # dfsproc=DFSprocess(workers_num, workers_slot, ratio, JobGraph, "default", processPoolPara)
        optimals, plans = dfsproc.process()
