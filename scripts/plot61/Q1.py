...
customFolder = "../q1/q1_custom"
evenFolder = "../q1/q1_even"
randomFolder = "../q1/q1_random"
...
with os.scandir(customFolder) as entries:
    ...
    for entry in entries:
        ...
        # customCPUsum.append(sum(cpuByNode)/100)
...
with os.scandir(evenFolder) as entries:
    ...
    for entry in entries:
        ...
        #     cpulist = cpu[ip]
        #     cpuByNode.append(sum(cpulist)/len(cpulist))
        # evenCPUsum.append(sum(cpuByNode)/100)
...
with os.scandir(randomFolder) as entries:
    ...
    for entry in entries:
        ...
        #     cpulist = cpu[ip]
        #     cpuByNode.append(sum(cpulist)/len(cpulist))
        # randomCPUsum.append(sum(cpuByNode)/100)
...
# ax[0].text(x=-0.55, y=0.5, s='(a) Throughput', rotation=90, va='center', ha='right', fontsize=labelSize, transform=ax[0].transAxes)
...
# ax[1].text(x=-0.55, y=0.5, s='(b) Backpressure', rotation=90, va='center', ha='right', fontsize=labelSize, transform=ax[1].transAxes)
...
plt.savefig('./q1-sliding.png', dpi=300)
plt.close()