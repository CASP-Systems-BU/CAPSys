...
# python3 rundynamic.py expjson/deem_unlimit_dynamic.json start custom_1
...
RECFGNXT=False      # whether next iteration need recfg
NEEDWARMUP=True    # whether current iteration need warmup
...
def rundynamic(_runiter):
    ...
    if(NEEDWARMUP):
        getMetricsWarmup(WARMUP, JobGraph, _runiter, vertex_ids, job_id)
    NEEDWARMUP=False
    ...
    if(_runiter>1):
        for vid in JobGraph.nodes:
            jg=pickle.load(open(RUNPATH+"/1_jg.pkl", "rb"))
    ...
    if(run1_top != run2_top):
        ...
        RECFGNXT=True
        NEEDWARMUP=True    # next iteration need warmup since target_input_rate changed
        return

    # decide whether need recfg for this iteration
    src_oop=0
    src_tops=0
    trigger=False
    total_parallelism = 0
...
#     if(abs(total_optimalparallelism - total_parallelism))>2:
#         trigger=True
    if(RECFGNXT):
        print("target_input_rate change in previous skipped iteration. will recfg    src_oop=",src_oop,"  src_tops=",src_tops,"  RECFGNXT=",RECFGNXT)
        RECFGNXT=False
    elif(src_oop/src_tops < TARGET_RATE_RATIO):
        print("target_input_rate not reached. will recfg    src_oop=",src_oop,"  src_tops=",src_tops,"  trigger=",trigger)
    else:
        # if target rate reached, ignore
        print("target_input_rate reached. no need recfg    src_oop=",src_oop,"  src_tops=",src_tops, "  total_optimalparallelism=",total_optimalparallelism, "  total_parallelism=",total_parallelism)
        return
    ...
    NEEDWARMUP=True    # next iteration need warmup since recfg happened
